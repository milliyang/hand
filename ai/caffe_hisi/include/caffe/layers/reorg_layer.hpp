...
#ifdef TRACK_ALL_LAYER2
        printf("reorg_cpu>> w:%d h:%d c:%d batch:%d stride:%d forward:%d out:%d\n", w, h, c, batch, stride, forward, out_c);
        printf("reorg_cpu>> first input:%f %f %f\n",x[0], x[1], x[2]);
#endif
        for(b = 0; b < batch; ++b){
            for(k = 0; k < c; ++k){
                for(j = 0; j < h; ++j){
...
#ifdef TRACK_ALL_LAYER2
        printf("reorg_cpu>> first out%f %f %f\n",out[0], out[1], out[2]);
#endif
    }
